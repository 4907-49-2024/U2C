...
\begin{document}
    ...
    which requirements were skipped in our prototype in section~\ref{subsubsec:unsat-reqs}.
    ...
    \begin{longtable}{|l|p{2.6cm}|l|p{4.5cm}|c|}
        ...
        \hline
        12 & Worst Case Mitigation & Scalability & U2C should scale at a smaller rate than O($n^3$). The scaling factors should be agents, stimuli, and agent behaviors. & \cellcolor{gray!30}  \\
        \hline
    ...
    This section details the decisions taken relating to input requirements.
    The first decision to take is the specific diagram type to support because it has cascading impacts on other requirements.
    This decision impacts the language, the diagram type may be specific to one or a small set of languages.
    ...
    More details on this later when we explain how to make a well-formed State Diagram for our inputs in section~\ref{subsubsec:input-example}.
...
%        \endhead
    ...
    They map directly to abstract behaviors in C2KA\@.
    \textbf{Atomic States} are states which do not contain other states.
    % TODO: (add C2KA base figures after each paragraph?)

    Atomic States always have a \textbf{doActivity} property, as they are needed for C2KA Concrete Behaviors.
    ...
    This follows state diagram convention, but it does not allow for guards,and enforces input and output on all transitions.
    \textit{sequential transitions} are special transitions with this labelling format: \{in-and-out-stimuli\}.
